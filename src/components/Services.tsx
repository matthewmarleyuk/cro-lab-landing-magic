...
const Services = () => {
  ...
  } = useStaggeredAnimation(services, 100);
  return <section id="services" className="py-16 md:py-20 relative overflow-hidden">
      <div className="container px-6 mx-auto">
        <div ref={titleRef} className={`text-center max-w-2xl mx-auto mb-16 transition-all duration-700 delay-200 transform ${titleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="body-md text-muted-foreground">We offer end-to-end optimisation services designed to systematically improve your conversion rates and drive business growth.</p>
        </div>
        
        <div ref={servicesRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => <div key={service.title} className={`rounded-xl p-6 transition-all duration-500 transform text-white border border-white/10 shadow-md hover:shadow-lg hover:-translate-y-1 ${servicesVisible[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{
          ...
          backgroundColor: service.bgColor
              ...
              <ul className="space-y-2">
                {service.items.map((item, i) => <li key={i} className="flex items-center text-sm text-white/90">
