...
const Tools = () => {
  ...
  const {
    ...
    visibleItems: toolsVisible
  } = useStaggeredAnimation(tools, 100);
  return <section id="tools" className="py-16 relative overflow-hidden bg-white">
      <div className="container px-6 mx-auto">
        ...
        <div ref={toolsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {tools.map((tool, index) => <div key={tool.name} className={`rounded-xl transition-all duration-500 transform border hover:shadow-lg hover:-translate-y-1 ${toolsVisible[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{
