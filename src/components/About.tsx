...
const About = () => {
  ...
  const {
    ...
    visibleItems: featuresVisible
  } = useStaggeredAnimation(features, 150);
  return <section id="about" className="py-4 md:py-6 relative overflow-hidden">
      <div className="container px-6 mx-auto">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div>
            <div ref={titleRef} className={`transition-all duration-700 delay-200 transform ${titleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div ref={contentRef} className={`transition-all duration-700 delay-400 transform ${contentVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              ...
              <p className="body-md text-muted-foreground">Unlike agencies that rely on opinions or design trends, we take a rigorous, evidence-based approach. By combining quantitative analytics with qualitative user insights, we create optimisation strategies that deliver measurable business impact.</p>
            </div>
          </div>
          
          <div ref={featuresRef} className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {features.map((feature, index) => <div key={feature.title} className={`rounded-xl p-6 transition-all duration-500 transform text-white border border-white/10 shadow-md hover:shadow-lg hover:-translate-y-1 ${featuresVisible[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{
